...
4. リポジトリのGitHub Pages URL（Settings > Pages、またはPages公開Actions runの結果から
   確認する）を各端末のブラウザで開けるようにしておく（項目12のPWAインストール確認用）。
...
| 4 | stylus描画（対応機種のみ） | ペンで線を描きながら、手のひらを意図的に画面へ乗せる/動かす | 手のひら接触が線として描画されず、ペンのストロークも中断されない（palm rejectionが機能する） |
...
| 10 | オフライン再起動（インストール済みアプリ） | 機内モードにしてインストール済みAndroidアプリを再起動 | オフラインで起動し、直前の作業内容が保持される |
| 11 | 画面回転 | 端末を回転させる | レイアウトが崩れず、描画内容が失われない |
| 12 | ブラウザPWAインストール / オフライン | インストール済みAndroidアプリとは別に、端末のブラウザでGitHub Pages URLを開き、「ホーム画面に追加」等でPWAとしてインストール。その後機内モードにしてPWAを再起動 | ブラウザからPWAとしてインストールでき、service workerのオフラインキャッシュが機能し、直前の作業内容が保持される（項目10はインストール済みAndroidアプリ自身の起動確認であり、これだけではservice worker/PWAインストール自体の疎通は確認できないため別項目とする） |
